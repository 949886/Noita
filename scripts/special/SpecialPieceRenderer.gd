...
func setup(p_placement: SpecialChunkPlacement) -> void:
	...
	if sprite == null:
		...
		add_child(sprite)
	var img: Image = _build_special_image()
...
func _build_special_image() -> Image:
	...
	_fill_noise(img, rock, dark)
	_carve_room(img, def)
	_draw_structure_details(img, def, accent, dark)
	return img

func _fill_noise(img: Image, rock: Color, dark: Color) -> void:
	img.fill(rock)
...
func _carve_room(img: Image, def: SpecialChunkDef) -> void:
...
func _carve_horizontal_profile(img: Image, profile: Array[PieceSocket.Socket], offset_x: int, edge_y: int, from_top: bool) -> void:
...
func _carve_vertical_profile(img: Image, profile: Array[PieceSocket.Socket], edge_x: int, offset_y: int, from_left: bool) -> void:
	for i: int in range(profile.size()):
		var socket: PieceSocket.Socket = PieceSocket.from_value(profile[i])
		if not PieceSocket.is_open(socket):
			continue
...
func _draw_structure_details(img: Image, def: SpecialChunkDef, accent: Color, dark: Color) -> void:
	var w: int = img.get_width()
	var h: int = img.get_height()
	# Platforms/pillars are image details, not TileMap cells.
...
func _rock_color(biome_id: StringName, style: int) -> Color:
...
func _dark_color(biome_id: StringName, style: int) -> Color:
	if style == SpecialChunkDef.TransitionStyle.SNOW or biome_id == &"snow":
...
func _accent_color(def: SpecialChunkDef) -> Color:
	match def.chunk_kind:
		SpecialChunkDef.ChunkKind.TREASURE:
