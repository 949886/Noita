...
static func open_width(socket: Socket, unit_size: int) -> int:
	match socket:
		...
		ROOM:
			return int(unit_size * 0.68)
		SHAFT:
			return int(unit_size * 0.34)
		ANY:
...
static func opening_count(socket: Socket) -> int:
	match socket:
		DOUBLE_OPEN_SMALL:
			return 2
		OPEN_SMALL, OPEN_MEDIUM, OPEN_LARGE, ROOM, SHAFT, ANY:
			return 1
		_:
			return 0
...
static func open_patterns(socket: Socket, unit_size: int) -> Array[Vector2i]:
	...
	match socket:
		...
		ROOM:
			result.append(Vector2i(int(unit_size / 2), int(unit_size * 0.68)))
		SHAFT:
			result.append(Vector2i(int(unit_size / 2), int(unit_size * 0.34)))
		ANY:
